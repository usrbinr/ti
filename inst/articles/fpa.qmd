...
-  Clarify if we are using a "standard" calendar or non-standard variation (currently supports 5-4-4, 4-4-5 and 4-5-4 calendars) see [standard vs. non standard calendar article](https://usrbinr.github.io/fpa/articles/calendar_types.html) for more information
...
[Lubridate](https://lubridate.tidyverse.org/) is an excellent package and is is at the core of many of fpa's functions. The issue isn't lubridate but rather the challenges and issues in your dataset that typically don't allow you to directly use lubridate. 
...
To correct this, `fpa` will automatically complete your calendar for each group for the missing periods to ensure there are no missing periods when calculating trends.
...
This package does the second option to  ensure we don't loose any of January's sales but to help flag for imbalance, fpa will add a column to let you know how many periods (eg. days) are in your comparison period to increase transparency to this dynamic.
...
If your data isn't already in a database then fpa will leverage duckdb to convert your data to enable larger than memory calculation. 
...
If your data is already in database, then fpa will use [dbplyr](https://dbplyr.tidyverse.org/) and will convert the functions to SQL to write the queries.
